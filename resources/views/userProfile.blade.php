    ...
    <form action="#" class="main_pro grid ">
        ...
        <div class=" i_two mb-5 mt-5">
            ...
            <div class="flex h-8 justify-center items-center btn mt-1 bg-yellow-200 hover:bg-green-400">
                <a download="{{auth()->user()->f_name."-".auth()->user()->l_name}} qualification "
                   href="{{url(auth()->user()->user_qualification_path)}}" class="w-full h-full flex justify-center
                ...
                <table class="w-full text-md bg-white shadow-md rounded mb-4">
                    ...
                    @foreach($aly as $key => $value)
                        ...
                        <tr class="border-b hover:bg-orange-100 bg-gray-100">
                            ...
                            <td class="p-3 px-5">{{$value['c_name']}}</td>

                            <td class="p-3 px-5">{{$value['app_status']}}</td>

                            <td class="p-3 px-5">{{$value['payment_status']}}</td>

                            <td class="p-3 px-5 flex justify-end">

                                @if($value['app_status'] == 'Approved' and auth()->user()->is_admin == false)
                                    <button type="button"
                                            class="mr-3 text-sm bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline">
                                        <a href="{{route('payPage')}}">Pay</a>
                                    </button>
                                @endif

                                @if($value['app_status'] == 'pending' and auth()->user()->is_admin)
                                    <button type="button"
                                            class="mr-3 text-sm bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline">
                                        <a href="{{route('updateCoursePage',[$value['pivot']['app_id'].'y'])}}">Approve</a>
                                    </button>

                                    <button type="button"
                                            class="text-sm bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline">
                                        <a href="{{route('updateCoursePage',[$value['pivot']['app_id'].'n'])}}">Decline</a>
                                    </button>
                                @endif
